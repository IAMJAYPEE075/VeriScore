...
def test_detect_solana_chain() -> None:
    client = TestClient(app)

    response = client.get("/detect/9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM")
    assert response.status_code == 200
    assert response.json()["chain"] == "solana"


def test_detect_best_evm_chain(monkeypatch) -> None:
    async def fake_detect(wallet_address: str):
        return (
            "base",
            [
                api.ChainCandidate(chain="base", total_transactions=18, unique_protocols=3, score=64),
                api.ChainCandidate(chain="ethereum", total_transactions=0, unique_protocols=0, score=10),
            ],
        )

    monkeypatch.setattr(api, "detect_best_evm_chain", fake_detect)
    client = TestClient(app)

    response = client.get(f"/detect/{ETH_WALLET}")
    assert response.status_code == 200
    payload = response.json()
    assert payload["chain"] == "base"
    assert payload["candidates"][0]["chain"] == "base"


def test_detect_best_evm_chain_prefers_score_when_tx_probe_is_saturated(monkeypatch) -> None:
    monkeypatch.setenv("ETHERSCAN_API_KEY", "test-key")

    probe_results = {
        "ethereum": api.ChainCandidate(
            chain="ethereum",
            total_transactions=100,
            unique_protocols=50,
            score=51,
        ),
        "base": api.ChainCandidate(
            chain="base",
            total_transactions=0,
            unique_protocols=0,
            score=10,
        ),
        "arbitrum": api.ChainCandidate(
            chain="arbitrum",
            total_transactions=100,
            unique_protocols=38,
            score=45,
        ),
        "optimism": api.ChainCandidate(
            chain="optimism",
            total_transactions=0,
            unique_protocols=0,
            score=10,
        ),
        "polygon": api.ChainCandidate(
            chain="polygon",
            total_transactions=100,
            unique_protocols=75,
            score=39,
    ...
    async def fake_probe(wallet_address: str, chain: str, etherscan_api_key: str) -> api.ChainCandidate:
        return probe_results[chain]

    monkeypatch.setattr(api, "probe_evm_chain", fake_probe)

    detected_chain, candidates = asyncio.run(api.detect_best_evm_chain(ETH_WALLET))

    assert detected_chain == "ethereum"
